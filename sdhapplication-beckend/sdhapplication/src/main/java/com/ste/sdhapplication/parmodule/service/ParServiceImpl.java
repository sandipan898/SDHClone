...
public class ParServiceImpl implements ParService {
    ...
    public ParModel getParameterById(Long parId) {
        return parRepository.findById(parId).orElse(null);
    }
    ...
    public HashMap<String, String> createParameter(ParModel parModel) {
        ...
        try {
            if (parRepository.findById(parModel.getParId()).isPresent())
            ...
            createParMap.put("ParId", createdPar.getParId().toString());
            createParMap.put("Error", null);
        } catch (Exception e) {
            createParMap.put("Status", "KO");
            createParMap.put("Message", "Error Creating Parameter!");
            createParMap.put("ParId", null);
            createParMap.put("Error", e.getMessage());
        }
        return createParMap;
    }

    public HashMap<String, String> updateParameter(ParModel parModel, Long parId) {
        var updateParMap = new HashMap<String, String>();
        parModel.setParId(parId);
        try {
            if(parRepository.findById(parId).isEmpty())
    ...
    public HashMap<String, String> deleteParameter(Long parId) {
        var deleteParMap = new HashMap<String, String>();
        try {
            if(parRepository.findById(parId).isEmpty())
                throw new Exception("Given Parameter Id does not exist!");
            parRepository.deleteById(parId);
